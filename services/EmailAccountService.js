...
class EmailAccountService {
    ...
    static async createAccount(email, password, domain, quota) {
        ...
        if (!fs.existsSync(mailboxPath)) {
            try {
                execSync(`mkdir -p "${path.join(mailboxPath, 'cur')}" "${path.join(mailboxPath, 'new')}" "${path.join(mailboxPath, 'tmp')}" 2>/dev/null`);
                execSync(`chown -R vmail:vmail "${mailboxBase}" 2>/dev/null`);
            } catch {}
        }
