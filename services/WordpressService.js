...
# END WordPress`;
        ...
        if (!fs.existsSync(wpCliGlobal)) {
            try {
                ...
                console.error('WP-CLI download failed:', e.message);
            }
        }
        ...
        if (!wpCmd) {
            return {
                success: true,
                method: 'browser',
                message: 'WP-CLI not available. Complete at: ' + siteUrl + '/wp-admin/install.php',
                installUrl: siteUrl + '/wp-admin/install.php'
            };
        }
    ...
    static removeWp(destDir) {
        if (fs.existsSync(destDir)) {
            // Only remove WordPress files, not the whole directory
            const wpItems = [
            ...
            // Recreate directory with proper ownership
            fs.mkdirSync(destDir, { recursive: true });
            try {
