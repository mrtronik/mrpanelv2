...
proses() {
    echo -ne "${CYAN}[⟳]${NC} $1..." | tee -a "$LOG_FILE"; 
}

sukses() {
    echo -e "\r${GREEN}[✓]${NC} $1" | tee -a "$LOG_FILE"; 
}

error() {
    echo -e "\r${RED}[✗]${NC} $1" | tee -a "$LOG_FILE"; 
    exit 1
}
...
echo "Proses instalasi dimulai."
echo "Silakan ngopi dulu, semua akan kami siapkan."
echo ""
